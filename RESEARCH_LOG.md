...
## 2026-08-19：N=3 临界锥与 `1e-3` 定量局部帽

- 直接把原方向树半径提高到 `1e-3` 会在二维临界方向附近爆炸。数值 SVD 显示
  活跃梯度差矩阵 `D` 的核正是临界空间；改用
  `||D u||_2 <= eta ||u||_2` 的梯度差临界锥后，`eta=0.3`、S-lemma 乘子
  `mu=2.9336` 在根处提供足够负曲率余量。
- 新增三阶纯有理 interval jet。完整六维半径 `1e-3` 源盒分成 `2^6=64`
  个子盒，每盒用 `U_x` 两端符号严格夹住移动底边暗点，并对包络 Hessian 的
  三阶导数取界。得到对称加权 Hessian 的谱变化上界
  `1.056728680784948458`。
- 将该谱界加到 S-lemma 矩阵对角线后，负矩阵六个顺序主子式的严格下界均正；
  第六个约 `1.62743724156698e13`。独立重算 64 个子盒和全部主子式逐字段一致。
- RTX 5090 用梯度差宽度选择切分坐标，9 秒量级生成 260,234 叶候选树。
  GPU 判定不受信任；16 进程纯 `Fraction` 重放耗时 1025.9 秒，失败 0。精确
  终止类型为 223,749 个单分支叶、2,210 个普通加权曲率叶、34,275 个临界锥叶。
- 因此已证明：对唯一 KKT 根 `A*`，
  `0 < ||A-A*||_infinity <= 1e-3` 时严格有 `I(A)<I(A*)`。这把既有显式
  完整六维半径扩大 10 倍。
- 全局 `P3<=7.57` 浮点 ROI 树在普通 `1e-3` 局部帽下从增长转为衰减：完整
  六层周期终点活跃盒为 435,167、257,755、17,142，第 66 层只剩 5,597；
  但这不是精确上界证书，且继续刷 `7.57` 的科学 ROI 低于研究全局等式结构。
- 另测“帽内 + 12 个帽外坐标薄片见证”混合叶。未条件化时看似能多关数百盒；
  扣除普通见证已经关闭的盒后，15,625 个根邻域盒只净增 8 个（根值阈值下
  16 个）。因此暂不把该终止类型扩展进精确全局验证器。
