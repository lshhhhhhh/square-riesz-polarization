...
- `E:\math\.venv` 的 PyTorch CUDA 环境随后已恢复可执行，RTX 5090 smoke test
  与正式搜索均已完成；精确 CPU 审计依赖已另行拆分，不再强制安装 PyTorch。
...
- 冻结的 19 位有效数字坐标见 `data/candidates/n03_symmetric.json`。257 网格加
  内部/四边多起点连续搜索数值上得到四角与下边中点五个活跃分支；审查后的
  精确字面值复核发现，截断坐标的角点实际低约 `3.19e-19`，严格五重等高属于
  100 位 KKT 根。
...
- 对冻结 `N=3` 坐标，spectral 用 141 次、componentwise 用 123 次分割均证明
  `I(X) >= 7.56838963`；角点 `(0,0)` 的精确有理求值给出
  `I(X) <= 7.568389640029690214292220397565988...`。
...
|---:|---:|---:|---:|---:|
| 29 | 272.495973646473 | 282.8569252861270 | 282.8 | 2298 / 2601 |
| 30 | 285.326742383550 | 285.3456853298848 | 285.34 | 3369 / 3400 |
| 31 | 294.208893270426 | 305.2983669115241 | 305.2 | 2391 / 2602 |
| 32 | 304.280799129879 | 317.2038189278292 | 317.1 | 2473 / 2732 |
| 33 | 311.665641329667 | 330.5954794801075 | 330.5 | 2788 / 3080 |
| 34 | 323.409928097309 | 337.9062358230380 | 337.8 | 2455 / 2751 |
| 35 | 329.708966808636 | 347.1957223312929 | 347.1 | 2432 / 2773 |

- 两种验证均使用 `Fraction` 对整个单位正方形做穷尽分支覆盖，不依赖浮点剪枝；
  七个目标全部通过。spectral 运行约 15--23 秒/例，componentwise 约
  45--65 秒/例。
...
- 独立 AI 未推翻任何定理级主张，并由 clean-room 标准库验证器完整重放
  `P_3<=7.58` 的 816 个根盒、1,304,240 个叶盒与精确 Kraft 覆盖；`N=3`
  两套固定构型下界、Krawczyk 包含以及抽样高 `N` 证书也均通过。
- 审查原文、脚本和输出先在提交 `2ef5eb9` 逐字冻结，再开始整改，避免用后改代码
  冒充独立审查时的实现。
- 接受的纠正包括：冻结的 19 位 `N=3` 坐标应以角点而非下边中点作最紧精确
  上界见证；高 `N` 旧基线统一向上舍入；大 `N` 搜索明确披露
  `incumbent-jitter`、`jitter=0.001` 与上游坐标初始化；CPU 审计依赖与 GPU
  搜索依赖分离。
- 审查把上游 `minimum_source_separation=0.005` 解读为硬约束，这一点被上游数据
  模式文档直接否定：该字段只是测得的描述统计，不参与边界或证明。近重合点仍是
  旧构型可能处在较差盆地的线索，但不能据此声称我们“移除了最小间距约束”。
- clean-room 验证器进一步去除 `assert`，并将最大叶界选择改为整数交叉乘法；这样
  `python -O` 也保留全部安全检查，且不再依赖浮点排序。

## 2026-08-19：投稿状态冻结与 N=3 活跃最暗点隔离

- 用户报告已在 Codex/仓库之外发送 Friedman 投稿邮件。仓库没有取得实际发件
  正文、最终署名、附件清单、时间戳或 message id，因此只记录
  `USER_REPORTED_EMAIL_SUBMITTED`，不把预发送草稿冒充实际邮件，也不表示对方已
  接收、认可或更新榜单。
- 新增 `square_riesz/local_proof.py` 与 `scripts/certify_n3_active_minima.py`。
  它们在 Krawczyk 根的 `1e-40` 对称参数盒外包上用纯 `Fraction` 区间算术验证
  观察点梯度、`U_xx` 和补集势下界。
- 四个角方块 `[0,1/8]`/`[7/8,1]` 的对应坐标导数严格指向角点；底部中央矩形
  `[3/8,5/8] x [0,1/8]` 上严格有 `U_y>0` 和 `U_xx>0`。结合对称恒等式
  `U_x(1/2,0)=0`，五个矩形各自只有一个极小点。
- 五个矩形外的 exact adaptive cover 仅需 35 次二分；对目标
  `t_upper+0.1` 全部闭合，最弱叶下界约 `7.69088827396`。与 A1 的等势 KKT
  方程合并，现已严格证明真对称 KKT 根的全局观察点极小集恰为四角和下边中点。
- 这一成果完成 A2 的核心计算机辅助部分，但不证明源位置空间中的局部/全局
  最优。下一步是 A3 的全六维五分支 epigraph 正则性与 A4 的移动边分支包络
  Hessian 区间负定。

## 2026-08-19：N=3 完整六维严格局部最优

- 把四个角点值和随源位置光滑移动的底边极小值写成五分支 epigraph。Krawczyk
  根的轨道乘子拆分为五个严格正乘子；反射对称性把三参数驻定方程精确提升到
  完整六个源坐标的加权驻定。
- 四个活跃梯度差的 interval `4x4` 主元子式约为 `-301377.4198543976`，严格
  远离零；所以差矩阵秩为 4，加入 epigraph 的公共 `t` 列后 LICQ 秩为 5，
  临界源方向空间为二维。
- 底边移动观察点的 exact `U_xx` 下界约 `35.62284`。用
  `U_AA-U_Ax U_xx^{-1} U_xA` 修正后，区间图基上的加权 `2x2` Hessian 对角
  上界约 `-3.95596,-21.25804`，行列式下界约 `84.03191`，严格负定。
- 与 A2 的五分支局部表示合并，标准非线性规划二阶充分条件证明：对称真根在
  三个有序源的完整六维空间中是严格局部最大点。该结论不声称全局最优；当前
  仍只有 `7.56838963<=P_3<=7.58`。
- 新增 `square_riesz/local_optimality.py`、
  `scripts/certify_n3_local_optimality.py`、精确 JSON 证书及交叉测试。数值广义
  特征值独立复现早先 SVD 诊断的 `-20.90257,-1.62793`，而最终判定仅用
  `Fraction`。
